...
HuskyHighlevelController::HuskyHighlevelController(ros::NodeHandle& nh)
    : nodeHandle(nh), subscriber(), vel_pub(), viz_pub(), stop_srv()
    , marker(), msg(), ctrl_p(.0) {
    ...
    stop_srv = nodeHandle.advertiseService("/start_stop",
        &HuskyHighlevelController::start_stop, this);
...
/* @brief: ROS service callback function 
 * start/stop the robot
 */
bool HuskyHighlevelController::start_stop(
